...
- T9.4 정성 내용 분석 개편: T9.1의 전수 탐지는 분석 대상의 경계를 정하는 용도로만
  ...
  **완료(2026-08-17):** 직접 언급 918건을 최신순부터 최초 수집일까지 모두 읽고,
  국가전략, 산업전환, 데이터센터, 집행, 개인정보, 저작권, 공공서비스, 허위콘텐츠,
  교육, 일자리, 국방의 11개 논점으로 정리했다. 자동 주제 막대, 월별 건수, 집중 날짜는
  공개 해석에서 제거하고 범위 설명과 최신순 전체 발언 검색만 유지했다.
...
- 근거 규칙: 정성 분석 문장은 등록된 발언 ID 목록과 함께 관리한다. 빌드할 때 모든
  ...
  **구현(2026-08-17):** 사람이 작성한 보고서를 `analytics/data/ai_qualitative.yaml`에
  두고 41개 대표 발언 ID와 근거 구절을 등록했다. 각 논점은 화자 3명 이상, 회의 3건
...
- 완료 기준: 최소 7개 주요 논점과 각 논점당 3건 이상의 고유 근거를 제시한다. 근거는
  ...
  **로컬 충족(2026-08-17):** 11개 논점, 대표 근거 41건, 전체 근거 918건을 렌더링했다.
  전체 테스트 100건이 통과했고, 1440px와 390px에서 핵심 판단부터 원문 근거까지의
  읽기 흐름을 확인했다. 전체 발언 검색은 `스마트글라스` 1건과 초기화 918건으로
  검증했으며 브라우저 콘솔 오류는 0건이었다. Pages 재배포 결과는 배포 완료 후 기록한다.
