...
현재 상태: **Phase 1 완료 + Phase 2 진행 중**. 국회 회의록 파싱 → 발언 추출 →
화자 매칭 → 주제·정책 입장 분류 → 인간 검수 큐 → 정적 사이트까지의 전체 루프가
동작합니다. 기본 경로는 LLM 없이 돌아가고, 주제와 입장 분류에 Claude 백엔드를
선택할 수 있습니다.
...
- **Phase 2 (진행 중)**: 인간 검수 큐, 입장 추출·변화 감지 ✅ / 다음 작업:
  표결 수집과 말–표 일치도
