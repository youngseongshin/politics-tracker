...
- T9.2 주제 분류: 각 발언을 산업·투자, 데이터센터·에너지, 인재·교육·노동,
  ...
  **대체(2026-08-17):** T9.4의 정성 분석을 정본으로 삼으면서 자동 주제 분류 코드와
  공개 태그를 제거했다. 직접 표현 탐지는 읽을 모집단의 경계에만 사용한다.
...
- T9.4 정성 내용 분석 개편: T9.1의 전수 탐지는 분석 대상의 경계를 정하는 용도로만
  ...
  선택지가 구체적으로 논의됐는가”로 바꾼다. AI 직접 언급 문맥을 모두 읽고 반복되는 문제의식,
  상반된 주장, 조건과 예외, 아직 답이 없는 질문을 사람이 코딩한다. 자동 주제 건수와
  월별 차트는 본문 해석에서 제거하고 자료 범위와 전체 발언 찾아보기만 부록으로 남긴다.
  **완료(2026-08-17):** 최초 공개본의 직접 언급 918건을 읽은 뒤 재배포에서 확인된
  직전 공개본 1,002건과 새 수집 결과를 ID로 대조해 모두 검토하고
  국가전략, 산업전환, 데이터센터, 집행, 개인정보, 저작권, 산업 지식재산, 공공서비스,
  허위콘텐츠, 교육, 일자리, 국방의 12개 논점으로 정리했다. 자동 주제 막대, 월별 건수,
  집중 날짜는 공개 해석에서 제거하고 범위 설명과 최신순 전체 발언 검색만 유지했다.
...
- 근거 규칙: 정성 분석 문장은 등록된 발언 ID 목록과 함께 관리한다. 빌드할 때 모든
  ...
  **구현(2026-08-17):** 사람이 작성한 보고서를 `analytics/data/ai_qualitative.yaml`에
  두고 현재 원문으로 재검증되는 47개 대표 발언 ID와 근거 구절을 등록했다. 각 논점은 화자 3명 이상, 회의 3건
  이상을 요구한다. 일부 ID나 근거 구절이 원문에서 사라지면 빌드를 실패시키며, 샘플처럼
  근거가 전혀 없는 자료에서는 해석을 숨긴다.
- 완료 기준: 최소 7개 주요 논점과 각 논점당 3건 이상의 고유 근거를 제시한다. 근거는
  ...
  **충족(2026-08-17):** 8월 12일까지 직전 공개본 1,002건과 새 수집 결과를 대조해
  검토한 보고서에 12개 논점과 현재 원문으로 재검증되는 대표 근거 47건을 등록했다.
  모든 근거의 ID·구절·출처를 최신 공개 회의록 문맥에 다시
  대조했으며, 각 논점의 화자 3명·회의 3건 조건을 통과했다. 전체 테스트 100건과 GitHub
  CI가 통과했다. 첫 Pages 배포에서 최신 데이터 증가를 발견한 뒤 보고서 범위를 갱신했으며,
  최종 [Pages 배포](https://github.com/youngseongshin/politics-tracker/actions/runs/31970543930)가
  성공했다. 공개본에는 해당 실행에서 수집한 AI 직접 언급 978건, 정성 논점 12개, 대표
  근거 47건이 표시된다. 1440px와 390px에서 읽기 흐름을 확인했고 모바일 가로 넘침,
  브라우저 콘솔 오류·경고는 각각 0건이었다. `서·논술형` 검색 6건과 대표 근거 원문
  펼치기도 확인했다.
